...
img_Array[i].src=option.imageDir + "/" + i+"."+options.imageExt;
clear();
...
image=new Image();
		...
		image.src=options.imageDir + "/" + "1."_options.imageExt;
...
if(options.imageCount<=0){alert("Provide a IMAGE COUNT");}
if(options.canvasID==""){alert("Provide a CANVAS ID");}
if(options.zoomPower<=0){alert("Zoom Power greater than 0");}
if(options.zoomRadius<50){alert("Provide a zoom Radius of at least 50 or more");}
if(options.imageExt==""){alert("Provide a Image extension. ex: png,jpg");}
if(options.canvasWidth==""){alert("Provide a Image Directory");}
...
view360.find("#"+options.canvasID).mousedown(function(e){
	bMouseDown = true;
	if(options.autoRotate==true_{stop_auto_rotate();}
	var canvasOffset = $(canvas).offset();
	iMouseX = Math.floor(e.pageX - canvasOffset.left);
	iMouseY = Math.floor(epageY - canvasOffset.top);
